		...
		spawn(function()
			while wait(1) do
				if shared.Toggle_AutoSell then
					for i,v in pairs(game:GetService("Workspace")["Robot World"].Sell:GetChildren()) do
						for i,v in pairs (v:GetChildren()) do
							if v.Name == "Script" then
								v.Parent.CFrame = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
								wait()
								v.Parent.CFrame = CFrame.new(12.8995, -31.9061, 7144.69)
							end
						end
					end
				end
...
local Teleport = main:CreateCategory("Teleport")
	local TS = Teleport:CreateSection("Teleport Stuff")
		TS:Create(
			...
			{
				options = {
					"Spawn", "Winter World", "Lava World", "Brick World", "Candy World", "MushRoom World", "Robot World"
				},
				-- Optional
